...
import com.anothermovieapp.managers.MovieDatabaseContract.PopularMovies
...
class PopularMoviesGridViewFragment : MoviesGridViewFragment() {
    ...
    override fun onCreateLoader(id: Int, args: Bundle?): Loader<Cursor> {
        val sortOrder = PopularMovies.COLUMN_PAGE + " ASC, " +
                PopularMovies.COLUMN_POSITION + " ASC"
        val uri = PopularMovies.CONTENT_URI
        ...
        if (isVisibleToUser) {
            ...
            if (activity != null) {
                ...
                if (supportActionBar != null) {
                    supportActionBar.title = context!!.resources.getString(R.string.popular_tab_name)
    ...
    override fun handleMessage(msg: Intent) {
        if (msg.getStringExtra(Constants.FRAGMENT_TAG) != Constants.POPULAR) {
            return
        }
    ...
    override fun fetchNewItems() {
        ...
        updateProgressBarVisibility()
        SyncMovieService.Companion.startFetchMovies(context, Constants.POPULAR, pageToLoad.toString() + "")
    }

    companion object {
        private const val POPULAR_MOVIE_LOADER = 1
    }
}